...
{:then polls}
  <div class="content home">
    <h2>Polls</h2>
    <p>
      Welcome to the voting app! You can view the list of existing polls below.
    </p>
    <table>
      <thead>
        <tr>
        ...
        <tbody
          >{#each polls as poll}
            <tr>
              ...
              <td class="actions">
                <a
                  href={votedPolls.includes(poll.id)
                    ? `/#/qa/${poll.id}/result`
                    : `/#/qa/${poll.id}`}
                  class="view"
                  title="View Poll"
                  >{votedPolls.includes(poll.id) ? "Results" : "Vote"}</a
                >
                <span
                  class="trash"
                  title="Delete Poll"
                  on:click={() => deletePoll(poll.id)}>Delete</span
                >
              </td>
            </tr>{/each}
        </tbody>
